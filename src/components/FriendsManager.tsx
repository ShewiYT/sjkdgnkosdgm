...
import { Users, Search, Play, Square, CheckCircle, XCircle } from 'lucide-react';
...
export default function FriendsManager({ accounts, selectedAccount }: FriendsManagerProps) {
  const {
    steamIdsToAdd, setSteamIdsToAdd, friendRequestLogs, friendRequestRunning,
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            ...
            filteredFriends.map(friend => (
              <div key={friend.steamId} className="flex items-center gap-2 px-3 py-2 hover:bg-white/5 transition-colors">
                <div className="relative">
      ...
      <div className="flex-1 p-6 overflow-y-auto space-y-6">
        <div>
          ...
          <p className="text-xs text-white/40 mt-1">
            Загрузите список Steam ID и аккаунты отправят им запросы
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              {!friendRequestRunning ? (
                ...
                <button onClick={stopFriendRequests}
          ...
          <div className="glass-card rounded-2xl overflow-hidden">
            <div className="px-4 py-3 border-b border-white/5">
              <h3 className="text-sm font-semibold text-white">Логи запросов</h3>
            </div>
                ...
                [...friendRequestLogs].reverse().map(log => (
                  <div key={log.id} className="px-4 py-2 flex items-center gap-2">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {log.error && <div className="text-[10px] text-white/40">{log.error}</div>}
                    </div>
                  </div>
                ))
              )}
