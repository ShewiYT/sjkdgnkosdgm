      ...
      <div className="glass-card rounded-2xl p-4 space-y-3">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 glass-input text-sm text-white px-3 py-2 rounded-xl outline-none"
          />
          <select
            value={newTarget}
            onChange={e => setNewTarget(e.target.value as 'panel' | 'api')}
            className="glass-input text-sm text-white px-3 py-2 rounded-xl outline-none bg-transparent"
          >
      ...
      <div className="space-y-3">
        {domains.length === 0 ? (
          <div className="glass-card rounded-2xl p-12 text-center">
            <Globe2 size={48} className="mx-auto mb-4 text-white/10" />
            <div className="text-sm text-white/30">Нет настроенных доменов</div>
          </div>
        ) : (
          domains.map(domain => (
