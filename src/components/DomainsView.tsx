...
export default function DomainsView() {
  ...
  const handleAdd = () => { if (!newDomain.trim()) return; addDomain(newDomain.trim(), newTarget); setNewDomain(''); };

  return (
    <div className="p-6 space-y-6 animate-fade-in overflow-y-auto max-h-[calc(100vh-52px)]">
      <div><h1 className="text-2xl font-semibold text-white flex items-center gap-2"><Globe2 size={24} /> Домены</h1><p className="text-sm text-white/40 mt-1">Управление доменами и SSL</p></div>
      <div className="glass-card rounded-2xl p-4 space-y-3">
        ...
        <div className="flex gap-2">
          ...
          <select value={newTarget} onChange={e => setNewTarget(e.target.value as 'panel' | 'api')} className="glass-input text-sm text-white px-3 py-2 rounded-xl outline-none bg-transparent"><option value="panel" className="bg-gray-900">Панель</option><option value="api" className="bg-gray-900">API</option></select>
      ...
      <div className="space-y-3">
        {domains.length === 0 ? (<div className="glass-card rounded-2xl p-12 text-center"><Globe2 size={48} className="mx-auto mb-4 text-white/10" /><div className="text-sm text-white/30">Нет доменов</div></div>) :
         domains.map(domain => (
          <div key={domain.id} className="glass-card rounded-2xl p-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div><div className="text-sm text-white font-medium">{domain.domain}</div><div className="text-[10px] text-white/30">{domain.target === 'panel' ? 'Панель' : 'API'} • {domain.ssl ? 'SSL ✓' : 'Без SSL'}</div></div>
            </div>
