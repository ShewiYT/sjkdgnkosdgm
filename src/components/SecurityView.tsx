...
export default function SecurityView({ accounts }: SecurityViewProps) {
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in overflow-y-auto max-h-[calc(100vh-52px)]">
      <div>
        <h1 className="text-2xl font-semibold text-white flex items-center gap-2"><Shield size={24} /> Безопасность</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="glass-card rounded-2xl p-4 bg-gradient-to-br from-blue-500/10 to-blue-600/5"><Lock size={20} className="text-blue-400 mb-2" /><div className="text-2xl font-bold text-white">{cleanCount}</div><div className="text-xs text-white/50">Чистых аккаунтов</div></div>
      ...
      <div className="glass-card rounded-2xl overflow-hidden">
        <div className="px-4 py-3 border-b border-white/5"><h3 className="text-sm font-semibold text-white">Детали безопасности</h3></div>
        <div className="divide-y divide-white/5">
          {accounts.map(acc => (
            <div key={acc.id} className="px-4 py-3 flex items-center gap-3">
              <div className="flex items-center gap-2 flex-1 min-w-0">
                {acc.avatarUrl ? <img src={acc.avatarUrl} alt="" className="w-6 h-6 rounded-full" /> : <span className="text-lg">{acc.avatar}</span>}
